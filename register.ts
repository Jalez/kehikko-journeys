 ...
 *
 *   bun run register            # or: PORT=7841 bun run register
 *
 ...
 *
 * ## The plugin writes this file too, and does not contradict that
 *
 ...
 * is not, and the difference is worth being exact about, because collapsing the
 * two loses something whichever way round you collapse them.
 *
 ...
 *
 * The ADDRESS is not a decision anybody made. Nobody chose 7840 — they chose to
 * be framed, and 7840 is a fact about where this process happened to bind, one
 ...
 * Start button that would put a second writer on the same `journeys.json`.
 ...
 *
 * The url is where to talk to this app; the directory is where to START it, and
 * it has to be a DIRECTORY rather than a command line for the reason `run.sh`
 * gives — a string a host handed to a shell would make this file a place to
 * write shell. With only the url, a host can frame a running module and can do
 * nothing at all about a stopped one: "nothing is answering" beside a Start
 ...
 * elsewhere registers itself correctly by being run. That is the one thing the
 * package cannot work out for itself, which is why it is still spelled here.
 *
 ...
 * silently.
 *
 * `registerAt` MERGES rather than overwrites, so a field somebody put beside the
